...
exports.findByGraphStatistics =async (graphId)=>{
  ...
  try {
    ...
    //최근 일주일 일별로 통계
    `SELECT DATE(tally.date) AS date, sum(match_history.attain_score) AS score FROM match_history RIGHT OUTER JOIN tally ON DATE(match_history.match_date) = DATE(tally.date) AND user_id ='${graphId}' 
    ...
    //최근 12개월 월별로 통계
    `SELECT date_format(DATE(tally.date), '%Y-%m') AS date, sum(match_history.attain_score) AS score FROM match_history RIGHT OUTER JOIN tally ON DATE(match_history.match_date) = DATE(tally.date) AND user_id ='${graphId}' 
